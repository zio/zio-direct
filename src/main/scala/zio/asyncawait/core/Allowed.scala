...
object Allowed {
  ...
  private def validateBlocksTree(using Quotes)(inputTree: quotes.reflect.Tree): Unit =
    import quotes.reflect._

    def validate(expr: Tree): Unit =
      //println(s"---------- Validating: ${Format.Tree(expr)}")
      expr match {
        // Do not allow declarations inside of async blocks
        case v: ClassDef =>
          Unsupported.Error.withTree(v, Examples.DeclarationNotAllowed)
        // defdefs are not allowed inside of async blocks unless they are auto-generated by scala
        // (scala does that when doing implicit-functions etc...)
        case v: DefDef
          if (!v.symbol.flags.is(Flags.Mutable) && !v.symbol.flags.is(Flags.Synthetic)) =>
          Unsupported.Error.withTree(v, Examples.DeclarationNotAllowed)
        case v: ValDef
          if (v.symbol.flags.is(Flags.Mutable)) =>
          Unsupported.Error.withTree(v, Examples.DeclarationNotAllowed)
    ...
    def validateTerm(expr: Term): Unit =
      expr match {
        ...
        // Enable a "lenient verification mode" that allows the below constructs so long as they do
        // not contain an "await" inside
        //case PureTree(_) =>
        ...
        // All the kinds of valid things a Term can be in async blocks
        ...
        case otherTree =>
          println(s"----------- Unsupported ${Format(Printer.TreeStructure.show(otherTree))}")
