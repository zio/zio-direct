...
trait WithAllowed {
  ...
  class Allowed(effectType: ZioEffectType) {

    object FromMutablePackage {
      def check(tpe: quotes.reflect.TypeRepr) = {
    ...
    def finalValidtyCheck(expr: Expr[_], instructions: Instructions) =
    ...
    def validateBlocksIn(expr: Expr[_], instructions: Instructions): Unit =
      given implicitInstr: Instructions = instructions
    ...
    private def validateBlocksTree(inputTree: quotes.reflect.Tree)(using instructions: Instructions): Unit =
      ...
      def validate(expr: Tree): Next =
        expr match {
          ...
          case term: Term if (effectType.isEffectOf(term.tpe)) =>
            Next.Exit
